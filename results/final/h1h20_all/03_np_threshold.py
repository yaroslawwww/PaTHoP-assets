    ...
    for r in donors:
        ...
        for _, row in df_t2.iterrows():
            exp2_records.append({
                ...
                'rmse_03': row[f'{r_str}_rmse'],
                'np_03': row[f'{r_str}_np'],
                'Donor_R_Val': r
            })
...
df_h1['np_03'] = df_h1['np_03'] * 100
df_h20['np_03'] = df_h20['np_03'] * 100
...
def plot_exp1_clear(df_h1, df_h10, df_h20, save_path='exp1_clear.png'):
    ...
    # RMSE
    plot_spline_safe(axes[0], df_h1['Recipient_Size'], df_h1['rmse_03'],
                     color='blue', marker='o', label='h=1', s=0.5)
    plot_spline_safe(axes[0], df_h10['Recipient_Size'], df_h10['rmse_03'],
                     color='green', marker='^', label='h=10', s=0.5)
    plot_spline_safe(axes[0], df_h20['Recipient_Size'], df_h20['rmse_03'],
    ...
    # NP
    plot_spline_safe(axes[1], df_h1['Recipient_Size'], df_h1['np_03'],
                     color='blue', marker='o', label='h=1', s=0.5)
    plot_spline_safe(axes[1], df_h10['Recipient_Size'], df_h10['np_03'],
                     color='green', marker='^', label='h=10', s=0.5)
    plot_spline_safe(axes[1], df_h20['Recipient_Size'], df_h20['np_03'],
                     color='red', marker='s', label='h=20', s=0.5)
...
def plot_exp2_mix(df_h1, df_h10, df_h20, save_path='exp2_mix.png'):
    ...
    for col, (df, h_label) in enumerate([(df_h1, 'h=1'), (df_h10, 'h=10'), (df_h20, 'h=20')]):
        ...
        for idx, r in enumerate(donors):
            ...
            y_raw = np.array(subset['rmse_03'].tolist() + [baseline['rmse_03']])
    ...
    for col, (df, h_label) in enumerate([(df_h1, 'h=1'), (df_h10, 'h=10'), (df_h20, 'h=20')]):
        ...
        for idx, r in enumerate(donors):
            ...
            if subset.empty:
                continue
            x_raw = np.array(subset['Recipient_Size'].tolist() + [10000])
            y_raw = np.array(subset['np_03'].tolist() + [baseline['np_03']])
            color = color_map.get(r, f'C{idx}')
            marker = markers[idx % len(markers)]
            plot_spline_safe(ax, x_raw, y_raw, color=color, marker=marker,
                             label=f'Donor r={r}', s=0.0, linewidth=2,
...
def plot_exp3_hybrid(df_h1, df_h20, save_path='exp3_hybrid.png'):
    ...
    for i, (df_full, h_label) in enumerate([(df_h1, 'h=1'), (df_h20, 'h=20')]):
        ...
        if not exp1.empty:
            plot_spline_safe(ax_rmse, exp1['Recipient_Size'], exp1['rmse_03'],
                             color='black', marker='', label='Ideal (Clear Data)', s=0.5,
                             linewidth=2, linestyle='--')
            plot_spline_safe(ax_np, exp1['Recipient_Size'], exp1['np_03'],
                             color='black', marker='', label='Ideal (Clear Data)', s=0.5,
                             linewidth=2, linestyle='--')
        ...
        for r in target_donors:
            ...
            plot_spline_safe(ax_rmse, subset['Total_Size'], subset['rmse_03'],
                             color=color, marker=marker, label=label,
                             s=2.0 if h_label == 'h=20' else 0.5,
                             linewidth=lw, linestyle=ls)

            # Исправлено: ax_rmse -> ax_np, rmse_03 -> np_03
            plot_spline_safe(ax_np, subset['Total_Size'], subset['np_03'],
                             color=color, marker=marker, label=label,
                             s=2.0 if h_label == 'h=20' else 0.5,
                             linewidth=lw, linestyle=ls)
