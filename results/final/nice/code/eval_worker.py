...
# Определяем метод из названия эксперимента
method = "apriori" if "APRIORI" in experiment_name.upper() else "dbscan"
...
print(f"[{experiment_name} h={horizon} | {method}] Rec={rec_size}, D_r={donor_r}, D_size={donor_size}")

# Формируем списки для evaluation
r_vals = []
ts_sizes =[]
...
# Запуск алгоритма с передачей метода
res = evaluation(r_values=r_vals, ts_sizes=ts_sizes, prediction_size=horizon, method=method)
