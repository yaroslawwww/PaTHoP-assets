...
os.makedirs('exp7_plots', exist_ok=True)
...
for suffix, alpha in alphas.items():
    ...
    for row_idx, h in enumerate(horizons):
        ...
        # 1. Filter Groups
        best_data = df[df['Experiment'] == 'EXP7_SELECTED']
        rand_data = df[df['Experiment'] == 'EXP7_RANDOM']

        # 2. Extract Baselines (Horizontal lines)
        base_10k = df[df['Experiment'] == 'EXP7_BASE_10K'][f'rmse_{suffix}'].mean()  # placeholders
        base_30k = df[df['Experiment'] == 'EXP7_BASE_30K'][f'rmse_{suffix}'].mean()
        ...
        fig_ind.suptitle(f'Experiment 7: Metric Validation (Horizon h={h}, α={alpha})', fontsize=16)
        fig_ind.tight_layout()
        fig_ind.savefig(f'exp7_plots/h{h}_alpha_{alpha.replace("/", "_")}.png')
        plt.close(fig_ind)
    ...
    fig_sum.suptitle(f'Experiment 7: Selected vs Random Donor Performance (α={alpha})', fontsize=18)
    fig_sum.tight_layout(rect=[0, 0.03, 1, 0.96])
    fig_sum.savefig(f'exp6_plots/summary_exp7_alpha_{alpha.replace("/", "_")}.png', dpi=200)
    plt.close(fig_sum)

print("Done. Validation results in 'exp7_plots'.")